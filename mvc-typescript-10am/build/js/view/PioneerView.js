...
export default class PioneerView extends Observer {
    ...
    constructor(subject) {
        super(subject);
        this.selector = document.querySelector('pioneers') ?? document.createElement('pioneers');
    }
    update() {
        this.render();
    }
    render() {
        this.selector.innerHTML = '<h1>Pioneers</h1>';
    }
    renderPioneerFromTemplate(computingPioneers) {
    ...
    renderPioneerFromJavascript(computingPioneers) {
        const rows = computingPioneers
            .map((pioneer) => {
