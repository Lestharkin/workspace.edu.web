...
export default class RetrieveMoviesCache implements RetrieveMoviesPort {
  ...
  constructor(
    ...
    private readonly cacheRepository: CacheRepository<StarwarsMovie[]>
  ) {
  ...
  public findAll = async (): Promise<Movie[]> => {
    const key = 'starwarsMovies'
    let starwarsMovies: StarwarsMovie[] | null = null

    if(await this.cacheRepository.isHealthy()) {
      starwarsMovies = await this.cacheRepository.get(key)
    }

    if (!starwarsMovies) {
      starwarsMovies = await this.starwarsAPI.fetchAllMovies()
    }
